...
function evaluatePathways(metrics) {
  ...
  const polyMissing = [];
  if (actual.G3 < 4) polyMissing.push('4 actual G3 subjects');
  if (equivalent.G2 < 5) polyMissing.push('1 additional G2-equivalent subject');
  if (!flags.hasEnglishG3) polyMissing.push('G3 English Language');

  cards.push(makePathway(
    'Polytechnic Year 1',
    polyEligible ? 'possible' : 'not-yet',
    polyEligible
      ? 'You meet the 4 G3 + 1 G2-equivalent threshold, including G3 English Language. Detailed cluster fit still matters, but G3 Mathematics is not compulsory in the cluster-fit view below.'
      : `Still needed: ${polyMissing.join('; ')}.`,
