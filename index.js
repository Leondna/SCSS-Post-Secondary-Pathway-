const SUBJECTS = [
  ...
  ["Malay (Special Programme)", ["G3"], ["language", "specialProgramme"]],
...
function isDesignMediaR2Subject(subjectName) {
  return [
    'Art',
    'Higher Art',
    'Music',
...
function computeMetrics() {
  ...
  const flags = {
    ...
    hasEnglishG3: false,
    hasMathG3: false,
    hasHumanitiesG3: false,
    hasSpecialProgrammeG3: false,
    hasJcMiHumanitiesOrSpecialProgramme: false,
    hasJcMiMath: false,
    hasArtsG3: false,
    hasBusinessG3: false,
    hasICTG3: false,
  ...
  state.selections.forEach(selection => {
    ...
    if (flagSet.has('humanities') && selection.level === 'G3') {
      flags.hasHumanitiesG3 = true;
    }
    if (flagSet.has('specialProgramme') && selection.level === 'G3') {
      flags.hasSpecialProgrammeG3 = true;
    }
    ...
    if (selection.level === 'G3' && isDesignMediaR2Subject(selection.name)) {
      flags.hasDesignMediaR2G3 = true;
    }
  });

  flags.hasJcMiHumanitiesOrSpecialProgramme = flags.hasHumanitiesG3 || flags.hasSpecialProgrammeG3;
  flags.hasJcMiMath = flags.hasMathG3;
...
function evaluatePathways(metrics) {
  ...
  const polyEligible = polyThreshold && flags.hasEnglishG3;
  const polyReason = polyEligible
    ? 'You meet the 4 G3 + 1 G2-equivalent threshold, and you have G3 English Language. Detailed cluster fit still matters, but G3 Mathematics is not compulsory in the cluster-fit view below.'
    : !polyThreshold
      ? 'Needs at least 4 actual G3 subjects and 1 additional G2-equivalent subject.'
      : 'You meet the 4 G3 + 1 G2-equivalent threshold, but Polytechnic Year 1 still requires G3 English Language.';

  cards.push(makePathway(
    'Polytechnic Year 1',
    polyEligible ? 'possible' : 'not-yet',
    polyReason,
    'See the Polytechnic cluster section below for a more detailed fit check.'
  ));

  const jcMiEligible = equivalent.G3 >= 5 && flags.hasEnglishG3 && flags.hasJcMiHumanitiesOrSpecialProgramme && flags.hasJcMiMath;
  const jcMiMissing = [];
  if (equivalent.G3 < 5) jcMiMissing.push('5 G3 subjects');
  if (!flags.hasEnglishG3) jcMiMissing.push('G3 English');
  if (!flags.hasJcMiHumanitiesOrSpecialProgramme) jcMiMissing.push('1 G3 Humanities or 1 G3 Special Programme subject');
  if (!flags.hasJcMiMath) jcMiMissing.push('Mathematics or Additional Mathematics at G3');
  const jcMiReason = jcMiEligible
    ? 'You meet the current subject conditions: 5 G3 subjects, G3 English, 1 G3 Humanities or 1 G3 Special Programme subject, and Mathematics or Additional Mathematics at G3.'
    : `Still needed: ${jcMiMissing.join('; ')}.`;

  cards.push(makePathway(
    'Millennia Institute (MI)',
    jcMiEligible ? 'eligible' : 'not-yet',
    jcMiReason,
...
function evaluatePolyClusters(metrics) {
  ...
  const totalG2G3 = actual.G2 + actual.G3;
  const hasB1 = actual.G3 >= 4;
  const hasB2 = totalG2G3 >= 5;
  const threshold = hasB1 && hasB2;

  const clusters = [
    {
      title: 'Arts & Humanities (ELR2B2-A)',
      checks: {
        'EL English Language': flags.hasEnglishG3,
        'R1 Relevant G3 subjects': flags.hasArtsG3,
        'R2 approved second relevant subject': flags.hasArtsG3 || flags.hasBusinessG3 || flags.hasDesignG3 || flags.hasScienceG3 || flags.hasICTG3,
        'B1 best other G3 subject': hasB1,
        'B2 best other G2/G3 subject': hasB2
      }
    },
    {
      ...
      checks: {
        'EL English Language': flags.hasEnglishG3,
        'R1 G3 Mathematics': flags.hasMathG3,
        'R2 Relevant G3 subjects': flags.hasArtsG3 || flags.hasBusinessG3,
        'B1 best other G3 subject': hasB1,
        'B2 best other G2/G3 subject': hasB2
      }
    },
    {
      ...
      checks: {
        'EL English Language': flags.hasEnglishG3,
        'R1 G3 Mathematics': flags.hasMathG3,
        'R2 Relevant G3 subjects': flags.hasScienceG3 || flags.hasICTG3 || flags.hasDesignG3,
        'B1 best other G3 subject': hasB1,
        'B2 best other G2/G3 subject': hasB2
      }
    },
    {
      ...
      checks: {
        'EL English Language': flags.hasEnglishG3,
        'R1 G3 Mathematics': flags.hasMathG3,
        'R2 Relevant G3 subjects': flags.hasDesignMediaR2G3,
        'B1 best other G3 subject': hasB1,
        'B2 best other G2/G3 subject': hasB2
      }
    }
  ...
  return clusters.map(cluster => {
    ...
    return {
      ...
      summary: likely
        ...
        : threshold
          ? 'You meet the B1/B2 count, but required subject conditions are still missing.'
          : 'You have not met the minimum conditions.'
    };
  });
}
...
function renderFlags(metrics) {
  const items = [
    ...
    ['Humanities subject at G3', metrics.flags.hasHumanitiesG3],
    ['Special Programme subject at G3', metrics.flags.hasSpecialProgrammeG3],
    ['JC/MI humanities or Special Programme condition', metrics.flags.hasJcMiHumanitiesOrSpecialProgramme],
...
function renderProgress(metrics) {
  const items = [
    ...
    ['Polytechnic G3 English requirement', metrics.flags.hasEnglishG3 ? 'Met' : 'Not yet', metrics.flags.hasEnglishG3 ? 'good' : 'warn']
  ];
